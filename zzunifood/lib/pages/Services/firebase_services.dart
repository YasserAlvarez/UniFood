...
Future<Map> getOrder(String idUser, int year, String month) async {
  ...
  print(startOfYearTimestamp);
  print(startOfNextYearTimestamp);
  ...
  QuerySnapshot querySnapshot = await FirebaseFirestore.instance
    ...
    .where('date', isLessThanOrEqualTo: startOfNextYearTimestamp)
    .get();
  // Lista de documentos filtrados por año y mes
  List<QueryDocumentSnapshot> filteredDocs = querySnapshot.docs;
  print(filteredDocs);
  // Consulta para filtrar por ID
  QuerySnapshot idQuerySnapshot = await FirebaseFirestore.instance
      .collection('ordenes').where('idUser', isEqualTo: idUser).get();
  // Lista de documentos filtrados por ID
  List<QueryDocumentSnapshot> idFilteredDocs = idQuerySnapshot.docs;
  // Intersecta las listas de documentos para obtener los documentos que cumplen con ambos filtros
  List<QueryDocumentSnapshot> queryOrder = filteredDocs.where((doc) => idFilteredDocs.contains(doc)).toList();
*/
